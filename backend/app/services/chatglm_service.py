...
3. 不要输出与答案无关的自我反思。"""
    ...
    def parse_solution_content(content: str) -> Dict:
        ...
        try:
            ...
            if ChatGLMService._has_solution_content(json_result):
                ...
                if not isinstance(json_result["steps"], list):
                    json_result["steps"] = ChatGLMService._normalize_steps_field(json_result["steps"])
                # 确保answer简洁明确
                json_result["answer"] = ChatGLMService._normalize_text_content(json_result["answer"])
                # 确保summary总结方法与易错点
                json_result["summary"] = ChatGLMService._normalize_text_content(json_result["summary"])
                return json_result
        except APIError:
            pass
        ...
        # 确保返回结构正确
        result["steps"] = [str(item).strip() for item in result["steps"] if str(item).strip()]
        result["thinking"] = ChatGLMService._normalize_text_content(result["thinking"])
        result["answer"] = ChatGLMService._normalize_text_content(result["answer"])
        result["summary"] = ChatGLMService._normalize_text_content(result["summary"])
